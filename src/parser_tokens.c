/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   parser_tokens.c                                    :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ikarjala <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/01/06 15:25:48 by ikarjala          #+#    #+#             */
/*   Updated: 2023/01/13 13:50:49 by ikarjala         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rtv1.h"

#if 0
int	check_token_num(char *word)
{
	const int	neg = word[0] == '-';
	int			dec;

	dec = 0;
	word += neg;
	while (*word)
	{
		if (ft_isdigit (*word))
			continue ; // word ++;
		else if (*word == '.' && !dec)
			dec = 1;
		else
		{
			// error
			return (0);
		}
		word ++;
	}
	return (1);
}

int	set_attr (char *word, void *ap, int *avrp)
{
	if (!check_token_num (word))
		return (parser_exception (EPARSE_TOKEN_UNKNOWN));
	if (!ap)
		return (parser_exception (EPARSE_TOKEN_INVALID));
	*ap = v;
	(*avrp) --;
}

#endif
static int	word_in_list(char *word, const char **list, const size_t count)
{
	int	n;

	n = -1;
	while (++n < (int)(count))
	{
		if (ft_strequ (word, (char *)(list[n])))
			return (n);
	}
	return (-1);
}

static inline t_obj	*obj_init(int id, t_mat *mat)
{
	t_obj	*obj;

	obj = (t_obj *)malloc(sizeof(t_obj));
	if (obj)
		*obj = (t_obj){.id = id,
			.pos = (t_vec){0, 0, 0}, .rot = (t_vec){0, 1, 0}, .r = 2,
			.mat = mat
		};
	return (obj);
}

int	token_try_obj(char *word, t_parser *p)
{
	const char	*o_words[] = {"sphere", "cylinder", "cone", "plane"};
	t_list		*node;
	int			n;

	n = word_in_list (word, o_words, sizeof(o_words) / sizeof(o_words[0]));
	if (n == -1)
		return (0);
#if 1
	node = p->obj;
	p->obj = (t_list *)malloc(sizeof(t_list)); // TODO: ft_lstpush (insert at beginning)
	p->obj->content = (void *)obj_init(n, p->default_matp);
	p->obj->content_size = sizeof(t_obj);
	p->obj->next = node;
	p->obj_count ++;
#endif
	p = NULL; // to shut the compiler up
	return (1);
}

#if 0
token_id	get_token_type(char *word)
{
	const char	*attr[] = {"pos", "euler", "radius"};
	const char	*mat[] = {"color", "gloss", "specular"};
	const char	*meta[] = {"cam", "ambient"};

	if (word_in_list (word, obj))
		return (obj); // collision !
	else if (word_in_list (word, attr))
		return (attr); // collision !
	else if (ft_strequ (word, "cam"))
		return (meta_obj);
	else if (ft_strequ (word, "ambient"))
		return (meta_attr);
	else
		return (null);
	return (0);
}
#endif
